...
while ! [ -f "$targetFile" ]; do    # While the targetFile does not exist, do this
    if ! [[ $targetFile = 'x' ]]; then        # Allow for exit option
        ...
        read -p 'Please enter an alternate directory/file or press x to exit: ' targetFile   # Prompt for alternate dir/file
    else
        exit 0
    fi
done        
...
# Third to sixth line replaces the first commas of each line in turn with Height, Width, Area, and Colour labels
